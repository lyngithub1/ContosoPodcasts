...
| Text-to-speech synthesis | ✅ (connected) / 🟡 (in-browser) | Connected mode calls Azure AI Speech and returns real MP3 audio. `POST /api/projects/:id/synthesize` renders the approved script to a multi-voice preview (per-speaker `<voice>` SSML) and stores the MP3 in Blob Storage; `GET /api/projects/:id/audio` streams it back from the private blob. **Long episodes are rendered in batches:** Azure Speech rejects any single request that would produce more than ~10 minutes of audio (HTTP 400, empty body), so `chunkSegmentsForSynthesis` splits the script into ≤7-minute batches and the returned CBR MP3 frames are concatenated. Verified on a 64-segment / 21-minute German script. In-browser mode uses the labeled browser-speech fallback (no audio file). |
...
| OneDrive / SharePoint delivery | ✅ (code) / ⚠️ (needs a Graph grant) | The `onedrive` publish channel is a **real** delivery adapter (`apps/api/src/onedrive.ts`): it uploads the episode MP3, transcript, and a disclosure sidecar into a configured drive folder via Microsoft Graph, using the platform managed identity (app-only, no secrets). Files >4 MiB go through a resumable upload session. It **fails closed** — the upload is attempted before the immutable publication is written, so a `PUBLISHED` project always means the artifact landed; on failure nothing is persisted and the project stays in `READY_TO_PUBLISH`. Disabled unless `GRAPH_DRIVE_ID` is set. **Owner action:** grant the identity `Sites.Selected` on the target site (see [docs/ONEDRIVE.md](docs/ONEDRIVE.md)). A zero-permission alternative for ad-hoc examples is `infra/export-episodes.ps1`, which writes to a locally-synced OneDrive folder. |
| Secure-email / webhook delivery adapters | 🟡 | Publishing is server-side: the API creates the **immutable** publication + per-recipient receipts in Cosmos and flips the project to `PUBLISHED`; no real email/webhook is sent. Use the `onedrive` channel above for delivery that actually happens. |
