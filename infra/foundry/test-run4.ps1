...
$o = Join-Path $env:TEMP 'foundry-testrun4.txt'
...
# List agents, show id + name for podcast-script-generator
$agents = (Call 'GET' "$proj/assistants?api-version=$av" $null)
"assistants call: ok=$($agents.ok) status=$($agents.status)" | Add-Content $o
if ($agents.ok) {
  $psg = ($agents.body | ConvertFrom-Json).data | Where-Object { $_.name -eq 'podcast-script-generator' } | Select-Object -First 1
  "psg.id=$($psg.id)  psg.name=$($psg.name)  model=$($psg.model)" | Add-Content $o
  $aid = $psg.id

  # Full run using assistant_id = agent id
  $thread = (Call 'POST' "$proj/threads?api-version=$av" '{}')
  $tid = ($thread.body | ConvertFrom-Json).id
  $msgBody = @{ role='user'; content='Reply with only this exact minified JSON: {"title":"t","segments":[{"speaker":"Host","heading":null,"directionCue":null,"text":"hello","claimIds":[]}]}' } | ConvertTo-Json -Depth 5
  Call 'POST' "$proj/threads/$tid/messages?api-version=$av" $msgBody | Out-Null
  $run = (Call 'POST' "$proj/threads/$tid/runs?api-version=$av" (@{ assistant_id = $aid } | ConvertTo-Json))
  "run w/ assistant_id(id): ok=$($run.ok) status=$($run.status)" | Add-Content $o
  if (-not $run.ok) { "run body: $($run.body)" | Add-Content $o }
  else {
    ...
    while ($st -in @('queued','in_progress','requires_action')) {
      ...
      $st = ((Call 'GET' "$proj/threads/$tid/runs/$rid?api-version=$av" $null).body | ConvertFrom-Json).status
    }
